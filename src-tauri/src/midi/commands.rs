...
use crate::launchpad::LaunchpadType;
use crate::midi::{get_input_devices, get_output_devices, identify_device};
use crate::midi::structs::{InputDevice, OutputDevice};

pub type EnumerateDevicesResult = Vec<(LaunchpadType, InputDevice, OutputDevice)>;

#[tauri::command]
pub fn enumerate_devices() -> EnumerateDevicesResult {
    let mut devices: EnumerateDevicesResult = vec![];

    if let Ok(inputs) = get_input_devices() {
        if let Ok(outputs) = get_output_devices() {
            for input in &inputs {
                let device_type = identify_device(input.idx.clone(), input.idx.clone() + 1);
                let output = outputs.iter().find(|e| e.idx == input.idx.clone() + 1).unwrap();
                println!("Got Device: {device_type:?}");
                if device_type != LaunchpadType::Unknown {
                    devices.push((device_type, input.clone(), output.clone()));
                }
