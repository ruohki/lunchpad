...
impl DeviceManager {
    ...
    pub fn simulate_control(&self, x: u8, y: u8, value: f32, kind: ControlKind, released: bool) -> MidiResult<()> {
        if self.active.is_none() {
            return Err(MidiError::NotConnected);
        }
        let event = ControlEvent { x, y, value: if value.is_finite() { value.clamp(0.0, 1.0) } else { 0.0 }, kind, released };
